<template>
  <div class="relative">
    ...
    <div
      ref="fretBoard"
      class="bg-[#331111] absolute inset-x-0 top-0 bottom-1 z-0 rounded-t-md"
    />
    <!-- nut styling -->
    <div
      ...
      <div v-for="fret in numberOfFrets" :key="fret" class="contents">
        ...
        >
          ...
          <div
            ...
            :style="{
              width: `${string < 3 ? '4' : string < 5 ? '3.5' : '3'}px`,
            }"
          ...
          >
            ...
            <div
              v-if="
                grip[string].fret == fret &&
                (!barreElementData ||
                  barreElementData.finger !== grip[string].finger)
              "
              ...
              :style="{
                boxShadow:
                  'inset 1px 0 6px 0 rgb(150 130 100 / 0.4), 1px 3px 4px #55555540',
              }"
            >
              <p class="text-[1.25rem] font-bold leading-none -translate-y-px">
                {{ grip[string].finger }}
              </p>
            </div>
            <!-- barre notation -->
            <div
              v-if="
                barreElementData &&
                barreElementData.fret == fret &&
                barreElementData.leftMostString == string
              "
          ...
          <div
            v-if="fret > 1"
            class="z-10 flex-1 h-1 border-b border-gray-900 bg-neutral-600"
          />
        </div>
      </div>
    </div>
...
const maybeStringGapWidth = 45.81

const setBarreElementData = () => {
  const stringData = Object.entries(props.grip)
    .map(([s, f]) => ({ string: parseInt(s), finger: f.finger, fret: f.fret }))
    .filter((s) => s?.finger && s?.fret)

  const barreFinger = stringData.find(
    (s) => stringData.filter((str) => s.finger == str.finger).length > 1
  )

  const allPositions = stringData.filter(
    (s) => s.finger == barreFinger?.finger
  )
  const allPositionsAreOnSameFret = allPositions.every(
    (f) => f.fret == barreFinger.fret
  )
  ...
  const leftMostString = stringData.find(
    (s) => s.string == Math.min(...barreStrings)
  )
  const { string: stringL, fret: fretL, finger: fingerL } = leftMostString
  const leftMostElement = document.getElementById(
    `${stringL}-${fretL}-${fingerL}`
  )

  const rightMostString = stringData.find(
    (s) => s.string == Math.max(...barreStrings)
  )
  const { string: stringR, fret: fretR, finger: fingerR } = rightMostString
  const rightMostElement = document.getElementById(
    `${stringR}-${fretR}-${fingerR}`
  )
  ...
  if (leftMostElement && rightMostElement) {
    ...
    const width =
      leftMostElement.offsetWidth +
      (stringGapElement.offsetWidth || maybeStringGapWidth) * stringSpan
